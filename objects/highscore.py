...
class HighscoreTableDrawer:
    def __init__(self, highscore_data, text = "Table",table_x:int = 100 ,table_y:int = 150, tittle_font_size:int = 30):
    ...
    def draw_table(self):
        ...
        for i in range(len(self.highscore_data)):
            pr.draw_text(f'player: {self.highscore_data[i]["name"]}, record: {self.highscore_data[i]["score"]}', 50, 200+(i*30), 25, pr.WHITE)